...
public:
	...
	//文字を描画
	static void text(int x, int y, int color, int font_handle, const std::string &str, int Alpha, int BlendMode = DX_BLENDMODE_ALPHA){
		SetDrawBlendMode(BlendMode, Alpha);
		DrawFormatStringToHandle(x + get().dx, y + get().dy, color, font_handle, "%s", str.c_str());
	}

	//指定した座標を中心に文字を描画
	static void centertext(int x, int y, int color, int font_handle, const std::string &str, int Alpha, int BlendMode = DX_BLENDMODE_ALPHA){
		SetDrawBlendMode(BlendMode, Alpha);
		int wd = GetDrawFormatStringWidthToHandle(font_handle, "%s", str.c_str());
		DrawFormatStringToHandle(x + get().dx - wd / 2, y + get().dy, color, font_handle, "%s", str.c_str());
	}

	//文字の右上の座標を指定して文字を描画
	static void righttext(int x, int y, int color, int font_handle, const std::string &str, int Alpha, int BlendMode = DX_BLENDMODE_ALPHA){
		SetDrawBlendMode(BlendMode, Alpha);
		int wd = GetDrawFormatStringWidthToHandle(font_handle, "%s", str.c_str());
		DrawFormatStringToHandle(x + get().dx - wd, y + get().dy, color, font_handle, "%s", str.c_str());
	}

	//指定した座標を中心に文字を描画 文字の間隔を指定可能
	static void centertext_space(int x, int y, int color, int font_handle, int space, const std::string &str, int Alpha, int BlendMode = DX_BLENDMODE_ALPHA){
		SetDrawBlendMode(BlendMode, Alpha);
	...
	//文字を描画
	static void text(const Vector2& pos, int color, int font_handle, const std::string &str, int Alpha, int BlendMode = DX_BLENDMODE_ALPHA){
		SetDrawBlendMode(BlendMode, Alpha);
		DrawFormatStringToHandle(pos.x + get().dx, pos.y + get().dy, color, font_handle, "%s", str.c_str());
	}

	//指定した座標を中心に文字を描画
	static void centertext(const Vector2& pos, int color, int font_handle, const std::string &str, int Alpha, int BlendMode = DX_BLENDMODE_ALPHA){
		SetDrawBlendMode(BlendMode, Alpha);
		int wd = GetDrawFormatStringWidthToHandle(font_handle, "%s", str.c_str());
		DrawFormatStringToHandle(pos.x + get().dx - wd / 2, pos.y + get().dy, color, font_handle, "%s", str.c_str());
	}

	//文字の右上の座標を指定して文字を描画
	static void righttext(const Vector2& pos, int color, int font_handle, const std::string &str, int Alpha, int BlendMode = DX_BLENDMODE_ALPHA){
		SetDrawBlendMode(BlendMode, Alpha);
		int wd = GetDrawFormatStringWidthToHandle(font_handle, "%s", str.c_str());
		DrawFormatStringToHandle(pos.x + get().dx - wd, pos.y + get().dy, color, font_handle, "%s", str.c_str());
	}

	//指定した座標を中心に文字を描画 文字の間隔を指定可能
	static void centertext_space(const Vector2& pos, int color, int font_handle, int space, const std::string &str, int Alpha, int BlendMode = DX_BLENDMODE_ALPHA){
		SetDrawBlendMode(BlendMode, Alpha);
		int str_num = (int)(str.size());
		if ((str_num % 2) == 0){
			for (int i = 0; i < (int)(str_num); i++){
...
public:
	...
	//半径rの円内部の点を返す
	static Vector2 RandInBox(Vector2 &center, const Vector2 &size){
		return center + Vector2(Random::Rand(-size.x / 2.0, size.x / 2.0), Random::Rand(-size.y / 2.0, size.y / 2.0));
	}

	//±
	static double range_float_rand(double max)
	{
		return FRand(max * 2) - max;
	}
...
public:
	...
	//追加
	//点と円
	static bool PointCircle(const Vector2& point_pos, const Vector2& circle_center, double radius)
	{
	...
	//点と矩形
	static bool BoxPoint(const Vector2& p_pos, const Vector2& box_pos, const Vector2& box_size){
	...
	*/
	...
	static DWORD GetColorHSV(int h, int s, int v){
		float f;
		int i, p, q, t;
		int rgb[3];
...
//	//スクリーンの中心を返す
//	inline static Vector2 GetScreenCenter(){
...
//	}
//	//ランダムな2文字の大文字アルファベットを返す
//	static std::string randStr(int loop){
//		std::string str;
...
//
//	//スクリーンから出ているかを返す関数
//	static bool isScreenOut(const Vector2 &pos){
//		return pos.x < 0 || pos.x > ScreenWidth
//			|| pos.y < 0 || pos.y > ScreenHeight;
//	}
//
//	//スクリーンからmarge外に出ているかを返す関数
//	static bool isScreenOutMarge(const Vector2 &pos, int marge){
